 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a repeated numbers") {
    assert(sum(List(1, 2, 4, 3, 0, 4, 2, 0, 2, 1)) === 19)
  }
  ...
  test("max throws an exception with an empty list") {
  ...
  test("max of a negative numbers") {
    assert(max(List(-1, -4, -9, -2)) === -1)
  }

  test("max of a repeated numbers") {
    assert(max(List(18, 8, 10, -1, 10, 18, 20, 2, 6, 20)) === 20)
  }

  test("max of an ordered list") {
    assert(max(List(1, 2, 3, 4, 5, 6, 21, 50)) === 50)
  }
}
