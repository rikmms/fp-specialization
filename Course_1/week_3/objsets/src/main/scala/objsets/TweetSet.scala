...
abstract class TweetSet {
  ...
  def mentions(keywords: List[String]): TweetSet = filter(t => keywords.exists(k => t.text.contains(k)))
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    try {
      (left union right).filter(p => p.retweets > elem.retweets).mostRetweeted
    }
    catch {
